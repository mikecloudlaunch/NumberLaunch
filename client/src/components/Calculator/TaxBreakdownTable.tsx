          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {taxResult.taxByBracket.map((bracket, index) => (
              <tr key={index}>
                ...
                <td className="px-6 py-3 whitespace-nowrap text-sm text-right text-gray-700 dark:text-gray-300">{bracket.rate === 30 ? '30.0¢' : bracket.rate === 0 ? '0.0¢' : bracket.rate === 19 ? '19.0¢' : bracket.rate === 37 ? '37.0¢' : bracket.rate === 45 ? '45.0¢' : `${bracket.rate.toFixed(1)}¢`}</td>
